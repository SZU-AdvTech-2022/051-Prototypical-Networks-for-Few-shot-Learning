...
class MiniImageNet(Dataset):
    ...
    def __init__(self, setname, train, test):
        ...
        if self.train:
            self.data = self.data[:int(0.0084 * imgs_num)]#5/6
            self.label = self.label[:int(0.0084 * imgs_num)]#5/6
        elif self.test:
            self.data = self.data[int(0.5 * imgs_num):]
            self.label = self.label[int(0.5 * imgs_num):]
        print(self.label)
        self.transform = transforms.Compose([
            ...
            transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                 std=[0.229, 0.224, 0.225])#使用Imagenet的平均值和标准差是一种常见的做法，它们是根据数百万张图片计算出来的
        ])
...
trainset = MiniImageNet('test5', train=True, test=False)
testset = MiniImageNet('test5', train=False, test=True)
print("——————————————————————————这是分割线——————————————————————————")
...
model.load_state_dict(torch.load('miniimagenet.pkl'))
'''这里读模型'''
# USEPreModel=True
...
if flag_m1:
    for param in model.parameters():
        param.requires_grad = False#设置参数不在求取梯度print("conv1.weights[0, 0, ...]:\n {}".format(resnet18_ft.conv1.weight[0, 0, ...]))# 3/3 替换fc层
model.classifier = torch.nn.Linear(640, 5)
print(model)
...
for epoch in range(10):
    train(epoch)
    test(epoch)
torch.save(model.state_dict(), 'miniimagenet_finetune.pkl')  # 保存模型
model.to("cpu")  # 转换回cpu